...
export function interpretMessage(message, currentDateTimeISO, senderJid, dataStore) {
    ...
    if (lowerCaseMessage.startsWith("nova tarefa:") ||
    ...
    // "remover" ou "apagar"
    if (lowerCaseMessage.startsWith("remover ") ||
    ...
    if (/^(evento:|novo evento:|adicionar evento:)?\s?.{2,} (hoje|amanhã|em \d+ dias|em \d+ (minutos|horas)|às \d{1,2}[:h]\d{2}|em \d{1,2}\/\d{1,2}(?:\/\d{4})?)/.test(lowerCaseMessage)) {
    ...
    if (["agenda", "tarefas", "eventos", "compromissos"].includes(lowerCaseMessage)) {
        return {
            type: "query",
            queryType: lowerCaseMessage === "tarefas" ? "tasks" :
                lowerCaseMessage === "eventos" ? "events" :
                    "both",
        };
    }
